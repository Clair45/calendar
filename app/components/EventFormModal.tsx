...
export default function EventFormModal({ visible, onClose, initialDate }: Props) {
  ...
  function pickDate(date: DateTime) {
    // 只改变日期部分，不改变时分秒
    const newStart = start.set({ year: date.year, month: date.month, day: date.day });
    const newEnd = end.set({ year: date.year, month: date.month, day: date.day });
    setStart(newStart);
    setEnd(newEnd);
    setDatePicked(true);
    setShowCalendar(false);
  ...
  function TimePicker({ visible, which, onClose }: { visible: boolean; which: "start" | "end"; onClose: () => void }) {
    ...
    return (
      <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
        <View style={styles.pickerBackdrop}>
          <View style={[styles.timePicker, { height: Math.min(420, SCREEN_H * 0.7) }]}>
  ...
  return (
    <>
                ...
                <View style={styles.verticalItem}>
                  ...
                  <View style={styles.timeControlRow}>
                    ...
                    <View style={styles.centerControls}>
                      <TouchableOpacity onPress={() => setShowCalendar(true)} style={styles.dateBtnFull}>
      ...
      <CalendarPicker visible={showCalendar} value={start} onPick={pickDate} onClose={() => setShowCalendar(false)} />
